...
* **Why is this better than using an animated GIF?**  
  Videos look kind of gross when converted to animated GIFs because of the colors sampling. Also the file size of video GIFs can get pretty huge. GIF is just not made for videos. JPG does a much better job of compressing video frames. Also, animated GIFs don't give you any playback controls. You can't pause a GIF or delay it's playback. With sketchapedia you can do that.

* **Why only "relatively short" videos?**  
  As you see further down, the container format for sketchapedia is a big image sprite of all the frames of each clip. Sadly, iOS limits the maximum image size (bigger image get sampled down), so that puts a limit on the maximum frames you can store.

* **Why is there no audio?**  
  sketchapedia doesn't support audio for the same reason animated GIF doesn't support audio either: because that's not what it was built for. (if you need audio, try [iphone-inline-video](https://github.com/bfred-it/iphone-inline-video))
...
You can use sketchapedia.js with AMD, CommonJS and browser globals.

```js
var sketchapediaControl = sketchapedia({
    selector : '.video',
    videos: {
        clip1: { src: 'clip1.jpg', frames: 38, cols: 6, loops: 1, onEnd: function(){
          console.log('clip1 ended.');
          sketchapediaControl.play('clip2');
        }},
    ...
    height: 400,
    loaded: function() {
        sketchapediaControl.play('clip1');
        // reverse playback
        // sketchapediaControl.play('clip1', true);
    }
});
```
If you want to use sketchapedia with [React](https://facebook.github.io/react/) you can check this simple [react + sketchapedia demo](http://codepen.io/moklick/pen/eJgbaL) to see how it works.
...
The sketchapedia function returns an object to control the video:

```js
var sketchapediaControl = sketchapedia(sketchapediaOptions);
```
...
```js
sketchapediaControl.play(videoKey [,isReverse, fps]);
```
...
```js
sketchapediaControl.resume();
```

**destroy**  
Stops video and removes the canvas of the current sketchapedia element from the DOM.

```js
sketchapediaControl.destroy();
```
...
```js
sketchapediaControl.getCurrentFrame();
```
...
```js
sketchapediaControl.setCurrentFrame(0);
```
...
## Is sketchapedia responsive?

Yes it is, thanks to a nice little trick. Regardless of what  `width` and `height` parameters you set in the sketchapedia constructor, you can use `style="width:100%"` on the canvas element and it will get scaled to the outer container and preserve its original aspect ratio.
