function sketchapedia() {
  ...
  if (touchy) {
    ...
    d3.select(window)
        .on("resize.sketchapedia", resize)
        .on("scroll.sketchapedia", reposition)
        .on("keydown.sketchapedia", keydown)
        .each(resize);
  ...
  function initialize(d, i) {
    this.__sketchapedia__ = {index: i, active: false};
  }

  function activate() {
    if (!this.__sketchapedia__.active) {
      this.__sketchapedia__.active = true;
      dispatchEvent({type: "activate"}, this.__sketchapedia__.index);
    }
  }

  function deactivate() {
    if (this.__sketchapedia__.active) {
      this.__sketchapedia__.active = false;
      dispatchEvent({type: "deactivate"}, this.__sketchapedia__.index);
    }
  }
  ...
  sketchapedia.size = function(_) {
    return arguments.length ? (size = [+_[0], +_[1]], resize(), sketchapedia) : size;
  };

  sketchapedia.scrollRatio = function(_) {
    return arguments.length ? (scrollRatio = +_, resize(), sketchapedia) : scrollRatio;
  };

  sketchapedia.fontSize = function(_) {
    return arguments.length ? (fontSize = +_, resize(), sketchapedia) : fontSize;
  };

  d3.rebind(sketchapedia, dispatch, "on");
