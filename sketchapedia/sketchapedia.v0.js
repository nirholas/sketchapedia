var sketchapedia = (function() {
  ...
  section.classed("sketchapedia", true).style("z-index", function(d, i) { return n - i; });
  ...
  // Sets the sketchapedia position.
  sketchapedia.position = function(y1) {
    ...
    if (y0 - y1) {
      self.on("scroll.sketchapedia", null);
      leap(y1);
      d3.select(body).transition()
          .duration(duration)
          .ease(ease)
          .tween("scrollTop", tween(yTarget = y1))
          .each("end", function() { yTarget = null; self.on("scroll.sketchapedia", scroll); });
    }
  ...
  self
      ...
      .on("scroll.sketchapedia", scroll)
      .on("mousemove.sketchapedia", snap)
      .on("hashchange.sketchapedia", hashchange);
  ...
  function hashchange() {
    ...
    if (!isNaN(hash) && hash !== yFloor) sketchapedia.position(hash);
  }
  ...
  function keydown() {
    ...
    sketchapedia.position(yTarget = Math.max(0, Math.min(n - 1, yTarget + delta)));
  ...
  function snap() {
    ...
    // else transition
    else if (y1 !== y0) sketchapedia.position(y1);
  }
  ...
  sketchapedia.duration = function(_) {
    if (!arguments.length) return duration;
    duration = _;
    return sketchapedia;
  };

  sketchapedia.ease = function(_) {
    if (!arguments.length) return ease;
    ease = _;
    return sketchapedia;
  };

  d3.rebind(sketchapedia, event, "on");
