 ...
 */
export const serverGatewayDeps = [
  { name: protocolPackageName satisfies ProtocolPackageName },
] as const;

export const serverGatewayPackageName = '@sketchapedia/server-gateway' as const;

export type ServerGatewayPackageName = typeof serverGatewayPackageName;
 ...
 * cross-package smoke tests until prompt 14 lands.
 */
export function resolveServerGatewayName(alias?: string | null): string {
  if (alias !== null && alias !== undefined && alias.length > 0) {
    return alias;
  }
  return serverGatewayPackageName;
}
