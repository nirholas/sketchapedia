/**
 * @sketchapedia/gpu-dispatcher
 *
 * GPU dispatch — Modal (primary) + RunPod adapter for model runtimes.
 *
 * Populated by prompt 22. This file is the scaffold stub produced by
 ...
 */
export const gpuDispatcherDeps = [
  { name: protocolPackageName satisfies ProtocolPackageName },
] as const;

export const gpuDispatcherPackageName = '@sketchapedia/gpu-dispatcher' as const;

export type GpuDispatcherPackageName = typeof gpuDispatcherPackageName;
 ...
 * cross-package smoke tests until prompt 22 lands.
 */
export function resolveGpuDispatcherName(alias?: string | null): string {
  if (alias !== null && alias !== undefined && alias.length > 0) {
    return alias;
  }
  return gpuDispatcherPackageName;
}
