/**
 * @sketchapedia/model-video
 *
 * Video model runtime — LTX-Video transition clips between keyframes.
 *
 * Populated by prompt 18. This file is the scaffold stub produced by
 ...
 */
export const modelVideoDeps = [
  { name: protocolPackageName satisfies ProtocolPackageName },
] as const;

export const modelVideoPackageName = '@sketchapedia/model-video' as const;

export type ModelVideoPackageName = typeof modelVideoPackageName;
 ...
 * cross-package smoke tests until prompt 18 lands.
 */
export function resolveModelVideoName(alias?: string | null): string {
  if (alias !== null && alias !== undefined && alias.length > 0) {
    return alias;
  }
  return modelVideoPackageName;
}
