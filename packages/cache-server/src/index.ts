/**
 * @sketchapedia/cache-server
 *
 * Server-side cache — Redis for metadata, S3/R2 for artifacts.
 *
 * Populated by prompt 20. This file is the scaffold stub produced by
 ...
 */
export const cacheServerDeps = [
  { name: cacheKeysPackageName satisfies CacheKeysPackageName },
  { name: protocolPackageName satisfies ProtocolPackageName },
] as const;

export const cacheServerPackageName = '@sketchapedia/cache-server' as const;

export type CacheServerPackageName = typeof cacheServerPackageName;
 ...
 * cross-package smoke tests until prompt 20 lands.
 */
export function resolveCacheServerName(alias?: string | null): string {
  if (alias !== null && alias !== undefined && alias.length > 0) {
    return alias;
  }
  return cacheServerPackageName;
}
