/**
 * @sketchapedia/edge-worker
 *
 * Cloudflare Worker — edge cache + CDN layer for scene artifacts.
 *
 * Populated by prompt 21. This file is the scaffold stub produced by
 ...
 */
export const edgeWorkerDeps = [
  { name: cacheKeysPackageName satisfies CacheKeysPackageName },
  { name: protocolPackageName satisfies ProtocolPackageName },
] as const;

export const edgeWorkerPackageName = '@sketchapedia/edge-worker' as const;

export type EdgeWorkerPackageName = typeof edgeWorkerPackageName;
 ...
 * cross-package smoke tests until prompt 21 lands.
 */
export function resolveEdgeWorkerName(alias?: string | null): string {
  if (alias !== null && alias !== undefined && alias.length > 0) {
    return alias;
  }
  return edgeWorkerPackageName;
}
